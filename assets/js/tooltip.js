...
})(window);function _isIPhoneOrIpodOrIpad(){return navigator.userAgent.match(/iPhone/i)||navigator.userAgent.match(/iPod/i)||navigator.userAgent.match(/iPad/i)
}function _isAndroid(){return navigator.userAgent.match(/Android/i)
}function configuraTooltips(){var a=new Tooltip($(".control-group:has(.restricoes)").find("input:not(#login), select"),$("#tip-holder"),"tip_left","element");
a.activate();
var b=new Tooltip($("#login"),$("#tip-holder"),"tip_left","element");
b.findTargetElement=function(c){return $(c).parent().find("#label-globomail")
};
b.activate()
}function removeMensagemErro(){if($(this).parent().hasClass("error")){$(this).parent().removeClass("error").find(".help-inline").remove()
}}function mostrarAguarde(){$.blockUI({message:$("#aguarde"),centerX:true,centerY:true,css:{border:"0px"},overlayCSS:{backgroundColor:"#fff",opacitykey:0.85}});
$("#aguarde").show()
}function ocultarAguarde(){$.unblockUI()
...
return this.each(function(){if(this.setSelectionRange){this.setSelectionRange(f,d)
...
k=l
}else{if(e[m]){i.push(new RegExp(e[m]));
if(h==null){h=i.length-1
}}else{i.push(null)
}}});
return this.trigger("unmask").each(function(){var u=c(this);
var p=c.map(f.split(""),function(y,x){if(y!="?"){return e[y]?j.placeholder:y
}});
var w=u.val();
function t(x){while(++x<=d&&!i[x]){}return x
}function q(x){while(--x>=0&&!i[x]){}return x
}function o(A,x){if(A<0){return
}for(var z=A,y=t(x);
z<d;
z++){if(i[z]){if(y<d&&i[z].test(p[y])){p[z]=p[y];
p[y]=j.placeholder
}else{break
}y=t(y)
}}s();
...
}}}}function r(A){var y=A.which;
if(y==8||y==46||(b&&y==127)){var B=u.caret(),z=B.begin,x=B.end;
if(x-z==0){z=y!=46?q(z):(x=t(z-1));
x=y==46?t(x):x
}m(z,x);
o(z,x-1);
return false
}else{if(y==27){u.val(w);
u.caret(0,n());
return false
}}}function v(A){var x=A.which,C=u.caret();
if(A.ctrlKey||A.altKey||A.metaKey||x<32){return true
}else{if(x){if(C.end-C.begin!=0){m(C.begin,C.end);
o(C.begin,C.end-1)
}var z=t(C.begin-1);
...
}}}function s(){return u.val(p.join("")).val()
}function n(y){var C=u.val();
var B=-1;
for(var x=0,A=0;
x<d;
x++){if(i[x]){p[x]=j.placeholder;
while(A++<C.length){var z=C.charAt(A-1);
if(i[x].test(z)){p[x]=z;
B=x;
break
}}if(A>C.length){break
}}else{if(p[x]==C.charAt(A)&&x!=k){A++;
B=x
}}}if(!y&&B+1<k){u.val("");
m(0,d)
}else{if(y||B+1>=k){s();
...
}u.data(c.mask.dataName,function(){return c.map(p,function(y,x){return i[x]&&y!=j.placeholder?y:null
}).join("")
});
if(!u.attr("readonly")){u.one("unmask",function(){u.unbind(".mask").removeData(c.mask.dataName)
}).bind("focus.mask",function(){w=u.val();
var y=n();
s();
var x=function(){if(y==f.length){u.caret(0,y)
}else{u.caret(y)
}};
(c.browser.msie?x:function(){setTimeout(x,0)
})()
}).bind("blur.mask",function(){n();
if(u.val()!=w){u.change()
}}).bind("keydown.mask",r).bind("keypress.mask",v).bind(a,function(){setTimeout(function(){u.caret(n(true))
},0)
})
}n()
})
 ...
 */
(function(){function a(j){if(/1\.(0|1|2)\.(0|1|2)/.test(j.fn.jquery)||/^1.1/.test(j.fn.jquery)){alert("blockUI requires jQuery v1.2.3 or later!  You are using v"+j.fn.jquery);
return
...
j.blockUI=function(q){e(window,q)
};
j.unblockUI=function(q){i(window,q)
};
j.growlUI=function(u,s,t,q){var r=j('<div class="growlUI"></div>');
if(u){r.append("<h1>"+u+"</h1>")
}if(s){r.append("<h2>"+s+"</h2>")
}if(t==undefined){t=3000
}j.blockUI({message:r,fadeIn:700,fadeOut:1000,centerY:false,timeout:t,showOverlay:false,onUnblock:q,css:j.blockUI.defaults.growlCSS})
};
j.fn.block=function(r){var q=j.extend({},j.blockUI.defaults,r||{});
this.each(function(){var s=j(this);
if(q.ignoreIfBlocked&&s.data("blockUI.isBlocked")){return
}s.unblock({fadeOut:0})
});
return this.each(function(){if(j.css(this,"position")=="static"){this.style.position="relative"
}if(j.browser.msie){this.style.zoom=1
}e(this,r)
})
};
j.fn.unblock=function(q){return this.each(function(){i(this,q)
})
};
j.blockUI.version=2.42;
j.blockUI.defaults={message:"<h1>Please wait...</h1>",title:null,draggable:true,theme:false,css:{padding:0,margin:0,width:"30%",top:"40%",left:"35%",textAlign:"center",color:"#000",border:"3px solid #aaa",backgroundColor:"#fff",cursor:"wait"},themedCSS:{width:"30%",top:"40%",left:"35%"},overlayCSS:{backgroundColor:"#000",opacity:0.6,cursor:"wait"},growlCSS:{width:"350px",top:"10px",left:"",right:"10px",border:"none",padding:"5px",opacity:0.6,cursor:"default",color:"#fff",backgroundColor:"#000","-webkit-border-radius":"10px","-moz-border-radius":"10px","border-radius":"10px"},iframeSrc:/^https/i.test(window.location.href||"")?"javascript:false":"about:blank",forceIframe:false,baseZ:1000,centerX:true,centerY:true,allowBodyStretch:true,bindEvents:true,constrainTabKey:true,fadeIn:200,fadeOut:400,timeout:0,showOverlay:true,focusInput:true,applyPlatformOpacityRules:true,onBlock:null,onUnblock:null,quirksmodeOffsetHack:4,blockMsgClass:"blockMsg",ignoreIfBlocked:false};
var c=null;
var h=[];
function e(u,G){var D,O;
var B=(u==window);
var x=(G&&G.message!==undefined?G.message:undefined);
G=j.extend({},j.blockUI.defaults,G||{});
if(G.ignoreIfBlocked&&j(u).data("blockUI.isBlocked")){return
}G.overlayCSS=j.extend({},j.blockUI.defaults.overlayCSS,G.overlayCSS||{});
D=j.extend({},j.blockUI.defaults.css,G.css||{});
O=j.extend({},j.blockUI.defaults.themedCSS,G.themedCSS||{});
x=x===undefined?G.message:x;
if(B&&c){i(window,{fadeOut:0})
}if(x&&typeof x!="string"&&(x.parentNode||x.jquery)){var J=x.jquery?x[0]:x;
var Q={};
...
if(Q.parent){Q.parent.removeChild(J)
}}j(u).data("blockUI.onUnblock",G.onUnblock);
var C=G.baseZ;
var N=(j.browser.msie||G.forceIframe)?j('<iframe class="blockUI" style="z-index:'+(C++)+';display:none;border:none;margin:0;padding:0;position:absolute;width:100%;height:100%;top:0;left:0" src="'+G.iframeSrc+'"></iframe>'):j('<div class="blockUI" style="display:none"></div>');
var M=G.theme?j('<div class="blockUI blockOverlay ui-widget-overlay" style="z-index:'+(C++)+';display:none"></div>'):j('<div class="blockUI blockOverlay" style="z-index:'+(C++)+';display:none;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>');
var L,H;
if(G.theme&&B){H='<div class="blockUI '+G.blockMsgClass+' blockPage ui-dialog ui-widget ui-corner-all" style="z-index:'+(C+10)+';display:none;position:fixed"><div class="ui-widget-header ui-dialog-titlebar ui-corner-all blockTitle">'+(G.title||"&nbsp;")+'</div><div class="ui-widget-content ui-dialog-content"></div></div>'
}else{if(G.theme){H='<div class="blockUI '+G.blockMsgClass+' blockElement ui-dialog ui-widget ui-corner-all" style="z-index:'+(C+10)+';display:none;position:absolute"><div class="ui-widget-header ui-dialog-titlebar ui-corner-all blockTitle">'+(G.title||"&nbsp;")+'</div><div class="ui-widget-content ui-dialog-content"></div></div>'
}else{if(B){H='<div class="blockUI '+G.blockMsgClass+' blockPage" style="z-index:'+(C+10)+';display:none;position:fixed"></div>'
}else{H='<div class="blockUI '+G.blockMsgClass+' blockElement" style="z-index:'+(C+10)+';display:none;position:absolute"></div>'
}}}L=j(H);
if(x){if(G.theme){L.css(O);
L.addClass("ui-widget-content")
}else{L.css(D)
}}if(!G.theme&&(!G.applyPlatformOpacityRules||!(j.browser.mozilla&&/Linux/.test(navigator.platform)))){M.css(G.overlayCSS)
}M.css("position",B?"fixed":"absolute");
if(j.browser.msie||G.forceIframe){N.css("opacity",0)
}var A=[N,M,L],P=B?j("body"):j(u);
j.each(A,function(){this.appendTo(P)
});
if(G.theme&&G.draggable&&j.fn.draggable){L.draggable({handle:".ui-dialog-titlebar",cancel:"li"})
}var w=f&&(!j.boxModel||j("object,embed",B?null:u).length>0);
if(g||w){if(B&&G.allowBodyStretch&&j.boxModel){j("html,body").css("height","100%")
}if((g||!j.boxModel)&&!B){var F=n(u,"borderTopWidth"),K=n(u,"borderLeftWidth");
var y=F?"(0 - "+F+")":0;
var E=K?"(0 - "+K+")":0
}j.each([N,M,L],function(t,T){var z=T[0].style;
z.position="absolute";
if(t<2){B?z.setExpression("height","Math.max(document.body.scrollHeight, document.body.offsetHeight) - (jQuery.boxModel?0:"+G.quirksmodeOffsetHack+') + "px"'):z.setExpression("height",'this.parentNode.offsetHeight + "px"');
...
if(E){z.setExpression("left",E)
}if(y){z.setExpression("top",y)
}}else{if(G.centerY){if(B){z.setExpression("top",'(document.documentElement.clientHeight || document.body.clientHeight) / 2 - (this.offsetHeight / 2) + (blah = document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop) + "px"')
}z.marginTop=0
}else{if(!G.centerY&&B){var R=(G.css&&G.css.top)?parseInt(G.css.top):0;
...
}if(x){if(G.theme){L.find(".ui-widget-content").append(x)
}else{L.append(x)
}if(x.jquery||x.nodeType){j(x).show()
}}if((j.browser.msie||G.forceIframe)&&G.showOverlay){N.show()
}if(G.fadeIn){var I=G.onBlock?G.onBlock:d;
var r=(G.showOverlay&&!x)?I:d;
var q=x?I:d;
if(G.showOverlay){M._fadeIn(G.fadeIn,r)
}if(x){L._fadeIn(G.fadeIn,q)
}}else{if(G.showOverlay){M.show()
}if(x){L.show()
}if(G.onBlock){G.onBlock()
}}m(1,u,G);
if(B){c=L[0];
h=j(":input:enabled:visible",c);
if(G.focusInput){setTimeout(p,20)
}}else{b(L[0],G.centerX,G.centerY)
}if(G.timeout){var v=setTimeout(function(){B?j.unblockUI(G):j(u).unblock(G)
},G.timeout);
j(u).data("blockUI.timeout",v)
}}function i(t,u){var s=(t==window);
var r=j(t);
...
m(0,t,u);
if(u.onUnblock===null){u.onUnblock=r.data("blockUI.onUnblock");
r.removeData("blockUI.onUnblock")
}var q;
if(s){q=j("body").children().filter(".blockUI").add("body > .blockUI")
}else{q=j(".blockUI",t)
}if(s){c=h=null
}if(u.fadeOut){q.fadeOut(u.fadeOut);
setTimeout(function(){l(q,v,u,t)
},u.fadeOut)
}else{l(q,v,u,t)
}}function l(q,t,s,r){q.each(function(u,v){if(this.parentNode){this.parentNode.removeChild(this)
}});
if(t&&t.el){t.el.style.display=t.display;
t.el.style.position=t.position;
if(t.parent){t.parent.appendChild(t.el)
}j(r).removeData("blockUI.history")
}if(typeof s.onUnblock=="function"){s.onUnblock(r,s)
}}function m(q,u,v){var t=u==window,s=j(u);
if(!q&&(t&&!c||!t&&!s.data("blockUI.isBlocked"))){return
}s.data("blockUI.isBlocked",q);
if(!v.bindEvents||(q&&!v.showOverlay)){return
}var r="mousedown mouseup keydown keypress";
q?j(document).bind(r,v,o):j(document).unbind(r,o)
}function o(u){if(u.keyCode&&u.keyCode==9){if(c&&u.data.constrainTabKey){var s=h;
var r=!u.shiftKey&&u.target===s[s.length-1];
var q=u.shiftKey&&u.target===s[0];
if(r||q){setTimeout(function(){p(q)
},10);
return false
}}}var t=u.data;
if(j(u.target).parents("div."+t.blockMsgClass).length>0){return true
}return j(u.target).parents().children().filter("div.blockUI").length==0
}function p(q){if(!h){return
}var r=h[q===true?h.length-1:0];
if(r){r.focus()
}}function b(w,q,A){var z=w.parentNode,v=w.style;
var r=((z.offsetWidth-w.offsetWidth)/2)-n(z,"borderLeftWidth");
var u=((z.offsetHeight-w.offsetHeight)/2)-n(z,"borderTopWidth");
if(q){v.left=r>0?(r+"px"):"0"
}if(A){v.top=u>0?(u+"px"):"0"
}}function n(q,r){return parseInt(j.css(q,r))||0
}}if(typeof define==="function"&&define.amd&&define.amd.jQuery){define(["jquery"],a)
}else{a(jQuery)
}})();(function(a){var b=function(c){var f=null;
var e=a("#"+c);
var d=a("input[name="+c+"]");
if(e!=undefined){f=e
}else{if(d!=undefined){f=d
}}return f
};
a.fn.autotab_magic=function(c){for(var d=0;
d<this.length;
d++){var f=d+1;
var e=d-1;
if(d>0&&f<this.length){a(this[d]).autotab({target:a(this[f]),previous:a(this[e])})
}else{if(d>0){a(this[d]).autotab({previous:a(this[e])})
}else{a(this[d]).autotab({target:a(this[f])})
}}if(c!=null&&(isNaN(c)&&c==a(this[d]).attr("id"))||(!isNaN(c)&&c==d)){a(this[d]).focus()
}}};
a.fn.autotab_filter=function(c){var e={format:"all",uppercase:false,lowercase:false,nospace:false,pattern:null};
if(typeof c=="string"||typeof c=="function"){e.format=c
}else{a.extend(e,c)
}for(var d=0;
d<this.length;
d++){a(this[d]).bind("keyup",function(g){var h=this.value;
switch(e.format){case"text":var f=new RegExp("[0-9]+","g");
h=h.replace(f,"");
break;
case"alpha":var f=new RegExp("[^a-zA-Z]+","g");
h=h.replace(f,"");
break;
case"number":case"numeric":var f=new RegExp("[^0-9]+","g");
h=h.replace(f,"");
break;
case"alphanumeric":var f=new RegExp("[^0-9a-zA-Z]+","g");
h=h.replace(f,"");
break;
case"custom":var f=new RegExp(e.pattern,"g");
var h=h.replace(f,"");
break;
case"all":default:if(typeof e.format=="function"){var h=e.format(h)
}break
}if(e.nospace){var f=new RegExp("[ ]+","g");
h=h.replace(f,"")
}if(e.uppercase){h=h.toUpperCase()
}if(e.lowercase){h=h.toLowerCase()
}if(h!=this.value){this.value=h
}})
}};
a.fn.autotab=function(c){var e={format:"all",maxlength:2147483647,uppercase:false,lowercase:false,nospace:false,target:null,previous:null,pattern:null};
a.extend(e,c);
if(typeof e.target=="string"){e.target=b(e.target)
}if(typeof e.previous=="string"){e.previous=b(e.previous)
}var d=a(this).attr("maxlength");
if(e.maxlength==2147483647&&d!=2147483647){e.maxlength=d
}else{if(e.maxlength>0){a(this).attr("maxlength",e.maxlength)
}else{e.target=null
}}if(e.format!="all"){a(this).autotab_filter(e)
}return a(this).bind("keydown",function(f){if(f.which==8&&this.value.length==0&&e.previous){e.previous.focus().val(e.previous.val())
}}).bind("keyup",function(g){var f=[8,9,16,17,18,19,20,27,33,34,35,36,37,38,39,40,45,46,144,145];
if(g.which!=8){var h=a(this).val();
if(a.inArray(g.which,f)==-1&&h.length==e.maxlength&&e.target){e.target.focus()
}}})
}
})(jQuery);jQuery.fn.customInput=function(){$(this).each(function(c){if($(this).is("[type=checkbox],[type=radio]")){var a=$(this);
var b=$("label[for="+a.attr("id")+"]");
var d=(a.is("[type=checkbox]"))?"checkbox":"radio";
$('<div class="custom-'+d+'"></div>').insertBefore(a).append(a,b);
var e=$('input[name="'+a.attr("name")+'"]');
b.hover(function(){$(this).addClass("hover");
if(d=="checkbox"&&a.is(":checked")){$(this).addClass("checkedHover")
}},function(){$(this).removeClass("hover checkedHover")
});
a.bind("updateState",function(){if(a.is(":checked")){if(a.is(":radio")){e.each(function(){$("label[for="+$(this).attr("id")+"]").removeClass("checked")
});
if(a.parents(".control-group").hasClass("error")){a.parents(".control-group").removeClass("error").find(".help-inline").remove()
}}b.addClass("checked")
}else{b.removeClass("checked checkedHover checkedFocus")
}}).trigger("updateState").click(function(){$(this).trigger("updateState")
}).focus(function(){b.addClass("focus");
if(d=="checkbox"&&a.is(":checked")){$(this).addClass("checkedFocus")
}}).blur(function(){b.removeClass("focus checkedFocus")
})
}})
...
}var isAValidLogin=function(a){if(a.charAt(0)=="."||a.charAt(a.length-1)=="."){return false
}if(a.charAt(0)=="_"||a.charAt(a.length-1)=="_"){return false
}if((a.length<4)||(a.length>20)){return false
}if(a.indexOf("@")!=-1){return false
}if(a.indexOf("..")!=-1){return false
}if(a.indexOf("__")!=-1){return false
}if(a.indexOf("_.")!=-1){return false
}if(a.indexOf("._")!=-1){return false
}if(a.indexOf(" ")!=-1){return false
}CHARS="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
for(i=0;
i<a.length;
i++){if(CHARS.indexOf(a.charAt(i))==-1&&a.charAt(i)!="."&&a.charAt(i)!="_"){return false
}}ALFA="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
for(i=0;
i<a.length;
i++){if(ALFA.indexOf(a.charAt(i))!=-1){return true
}}return true
};
var no_accents_alfanumeric_domain="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
var login_domain="abcdefghijklmnopqrstuvwxyz0123456789._";
var new_pass_domain="abcdefghijklmnopqrstuvwxyz0123456789";
var isNovaSenha=function(c){var b="";
var a=(window.Event)?c.which:c.keyCode;
b=String.fromCharCode(a);
if(a!=0&&a!=8&&a!=9&&a!=16&&a!=144){if(new_pass_domain.indexOf(b)==-1){return false
}}return true
};
var islogin=function(c){var b="";
var a=(window.Event)?c.which:c.keyCode;
b=String.fromCharCode(a);
if(a!=0&&a!=8&&a!=9&&a!=16&&a!=144){if(login_domain.indexOf(b)==-1){return false
}}return true
};
var onceClick=function(a){if(a.usado==undefined){a.usado=true;
return true
}return false
};
var soNumero=function(b){var a=(window.event)?event.keyCode:b.which;
if(((a<48)||(a>57))&&(a!=13)&&(a!="0")&&(a!=8)){if(window.event){event.keyCode=0
}return false
}return true
};
var allTrim=function(a){while(a.substring(0,1)==" "){a=a.substring(1,a.length)
}while(a.substring(a.length-1,a.length)==" "){a=a.substring(0,a.length-1)
}return a
};
var trim=function(a){while(a.substring(a.length-1,a.length)==" "){a=a.substring(0,a.length-1)
}return a
};
var leftTrim=function(a){while(a.substring(0,1)==" "){a=a.substring(1,a.length)
}return a
};
var block=function(a){document.getElementById(a).style.display="block"
};
var none=function(a){document.getElementById(a).style.display="none"
};$(function(){$('.box-como-resolver input[type="radio"]').click(function(){$(".box-alterar, .box-reenviar").addClass("oculta");
$(".box-"+$(this).val()).removeClass("oculta")
});
$(".opcao-reenviar #btnReenviar").click(function(){var a=$(".reenviar-text");
a.addClass("aguarde").text("aguarde...");
$("#btnReenviar").remove();
$.ajax({url:"/ReenviaEmailConfirmacaoCadastro",dataType:"text",success:function(b){a.removeClass("aguarde").addClass(b);
if(b=="sucesso"){a.addClass("ok").html("email reenviado com sucesso!")
}else{a.html("erro ao reenviar email!")
}}});
return false
});
$("#emailAlterado").keypress(function(){if($(".box-alterar").hasClass("sucesso")){$("#btnAlterar").show()
}});
$(".opcao-alterar #btnAlterar").click(function(b){b.preventDefault();
var a=$("#emailAlterado").val();
$(".box-alterar .help-inline").remove();
if(!a){$(".box-alterar").addClass("error").append('<span class="help-inline">ops, faltou preencher este campo.</span>');
$("#emailAlterado").focus();
return false
}$(this).addClass("oculta");
$(".box-alterar").removeClass("oculta error");
mostrarAguarde();
$.ajax({url:"/AlteraEmailCadastro",type:"POST",data:{email:a},dataType:"text",success:function(c){ocultarAguarde();
if(c=="sucesso"){$(".box-alterar").addClass(c).append('<span class="help-inline ok">e-mail alterado com sucesso!</span>');
$("#btnAlterar").hide();
$("#btnReenviar").val("reenviar email para "+a);
$(".user_email").text(a)
}else{if(c=="invalido"){$(".box-alterar").addClass("error").append('<span class="help-inline">ops, este e-mail parece um pouco estranho. Poderia conferir?</span>')
}else{$(".box-alterar").addClass("error").append('<span class="help-inline">Este e-mail j&aacute; est&aacute; cadastrado.</span>')
}}},error:function(c){ocultarAguarde();
$(".box-alterar").addClass("error").append('<span class="help-inline">Ops, alguma coisa n&atilde;o deu certo...<br/>Por favor, tente novamente.</span>')
}});
return false
});
$("#emailAlterado").keypress(function(){$("#btnAlterar").removeClass("oculta");
$(".alterarEmailResposta").addClass("oculta")
})
});function validaForm(a){if(a.txtSenha.value.length<6){alert("Ã‰ necessÃ¡rio ter mais de 6 dÃ­gitos na senha.");
return false
}}$(function(){$(".checkbox input, .radio input").customInput();
$("#data_de_nascimento").removeAttr("maxlength").mask("99/99/9999");
configuraTooltips();
$(".cw-link-cadastro").click(function(){window.location=window.urlCadastro
});
$(".cw-link-login").click(function(){window.location=window.urlLogin
});
$("#termo_uso_globocom, #termo_uso_globomailfree").change(function(){if($(this).is(":checked")){$("#cadastrar").prop("disabled",false)
}else{$("#cadastrar").prop("disabled",true)
}}).change();
$("input, select").change(removeMensagemErro);
$("#txtSenha").bind("keyup",removeMensagemErro);
$(".fechar_termouso").click(function(){$.unblockUI();
$("#txtTermo").html();
$(".componente-termo-uso-basico").data("servicoid",null);
return false
});
$(".exibe-termo-uso").click(function(){var a=$(this).attr("servicoid");
if($(".componente-termo-uso-basico").data("servicoid")==a){$(".componente-termo-uso-basico").hide().data("servicoid",null)
}else{$.get("/ajax/termoUso/"+a,function(e){$("#txtTermo").html(e);
var b=$(window).width();
b=_isIPhoneOrIpodOrIpad()?window.innerWidth:b;
b=_isAndroid()?screen.width:b;
var d=30;
var c=(b-parseInt(558,10))/2;
$.blockUI({message:$(".componente-modal.termos_uso"),css:{top:d,left:c,cursor:"auto",width:"558px",height:"auto",padding:0,margin:0,border:"4px solid #e5e5e5","border-radius":"5px",textAlign:"left","z-index":"999999"},overlayCSS:{backgroundColor:"#fff",opacitykey:0.85}})
});
if(a!=window.SERVICO_COMENTARIOS&&$(this).attr("mostra-termo-comentarios")!="false"){$.get("/ajax/termoUso/"+window.SERVICO_COMENTARIOS,function(b){$("#txtTermo-comentarios").html(b);
$("#txtTermo-comentarios").show()
})
}else{$("#txtTermo-comentarios").hide()
}$(".componente-termo-uso-basico").data("servicoid",a)
}return false
});
$("#abreAjuda").click(function(){$(".box-como-resolver").slideToggle("slow")
});
$("#botao-habilitar-openid").click(function(){var c=$(this);
var a=$("#chkHabilitarOpenID").attr("checked");
if(!a){if(confirm("VocÃª deseja habilitar seu OpenID?")){$("#chkHabilitarOpenID").attr("checked","checked")
}else{return false
}}$(".habilitar-openid").html("<span class='aguarde'><img src='/resources/img/cadastro/img_aguarde.gif' alt='Aguarde' /> <strong>aguarde...</strong></span>");
var b=jQuery.get("_include/habilitar-openid-sucesso.jsp",function(d){$(".habilitar-openid").html(d)
})
});
$(".cliqueUnico").click(function(a){if(this.ativo){$(this).attr("disabled","disabled")
}this.ativo=true
})
});$(document).ready(function(){$(".box-escolha-email :radio").click(function(){$(".box-escolha-email .botao-continuar").removeAttr("disabled");
$(".box-escolha-email .botao-continuar").removeClass("desabilitado");
$(".box-escolha-email .selecionada").removeClass("selecionada");
$(this).parent().parent().addClass("selecionada");
if($(".box-escolha-email :radio:eq(0)").attr("checked")){$("#opcao-escolher-email input").focus()
}});
$("#opcao-escolher-email input").focus(function(){$(".box-escolha-email .botao-continuar").removeAttr("disabled");
$(".box-escolha-email .botao-continuar").removeClass("desabilitado");
$(".box-escolha-email :radio").attr("checked","");
$(".box-escolha-email :radio:eq(0)").attr("checked","checked");
$(".box-escolha-email .selecionada").removeClass("selecionada");
$(this).parent().parent().addClass("selecionada")
});
if(!$(".box-escolha-email :radio:eq(0)").attr("checked")&&!$(".box-escolha-email :radio:eq(1)").attr("checked")){$(".box-escolha-email .botao-continuar").attr("disabled","disabled");
$(".box-escolha-email .botao-continuar").addClass("desabilitado")
}});$(function(){function a(d,b){var c="";
if(d.length>1){c='<option value = "0" > Selecione</option>'
}for(i in d){c+='<option value="'+d[i].id+'">'+d[i].nome+"</option>"
}$(b).html(c)
}$("#pais").change(function(){$("#estado").html('<option value="sda">Carregando...</option>');
var b=$(this).val();
$.getJSON("/ajax/estado/"+b,function(c){a(c.estados,"#estado")
});
$("#municipio").html('<option value="99999">N&atilde;o informada</option>')
});
$("#estado").change(function(){$("#municipio").html('<option value="sda">Carregando...</option>');
var b=$(this).val();
$.getJSON("/ajax/cidade/"+b,function(c){a(c.cidades,"#municipio")
})
});
$("#cep").autotab({format:"number"}).blur(function(c){var d=function(f,e,g){var h="";
for(i in g){h+='<option value="'+g[i].id+'">'+g[i].nome+"</option>"
}$(e).html(h);
$(e).val(f.estadoId).find("option[value="+f.estadoId+"]").attr("selected","selected")
};
var b=$("#cep").val();
if(b.length==8){$.getJSON("/ajax/cep/"+b,function(f){var e=f.cep;
$("#endereco").val(e.logradouro);
$("#bairro").val(e.bairro);
$("#pais").val(e.paisId);
$("#endereco, #bairro, #pais, #estado, #municipio").each(removeMensagemErro);
if(e.logradouro!=null&&e.bairro!=null){$("#endereco").prop("disabled",true);
$("#bairro").prop("disabled",true)
}else{$("#endereco").prop("disabled",false);
$("#bairro").prop("disabled",false)
}var g=$("#pais option:selected").text();
$.getJSON("/ajax/estado/"+e.paisId,function(h){d(e,"#estado",h.estados)
});
$.getJSON("/ajax/cidade/"+e.estadoId,function(h){d(e,"#municipio",h.cidades)
})
})
}});
if($("span[for='cep'].help-inline").length==0&&($("span[for='endereco'].help-inline").length>0||$("span[for='bairro'].help-inline").length>0)){$("#endereco, #bairro").prop("disabled",false)
}else{$("#endereco, #bairro").prop("disabled",true)
}$("#dddCelular").autotab({format:"number",target:"numeroCelular"});
$("#numeroCelular").autotab({format:"number"});
$("#dddComercial").autotab({format:"number",target:"numeroComercial"});
$("#numeroComercial").autotab({format:"number"});
$("#dddResidencial").autotab({format:"number",target:"numeroResidencial"});
$("#numeroResidencial").autotab({format:"number"})
});CustomSelect={personalizar:function(b,a){a=$.extend({styleClass:"custom-select",idPrefix:"select"},a);
b.each(function(){var c=$(this);
var f=$("<div />"),d=$('<span class="is-selected selected"><i></i><span class="custom-select-arrow"></span></span>');
f.addClass(a.styleClass);
if(a.useID){f.attr("id",a.idPrefix+"-"+c.attr("id"))
}c.css("opacity",0);
c.wrap(f);
c.before(d);
f=c.parent("div");
f.css("width",c.css("width"));
d=f.find("span.is-selected i");
var e=c.find(":selected:first");
if(e.length==0){e=c.find("option:first")
}d.html(e.text());
c.change(function(){d.text(c.find(":selected").text());
f.removeClass("active")
}).focus(function(){f.addClass("focus")
}).blur(function(){f.removeClass("focus");
f.removeClass("active")
}).mousedown(function(){f.addClass("active")
}).mouseup(function(){f.removeClass("active")
}).click(function(){f.removeClass("active")
}).hover(function(){f.addClass("hover")
},function(){f.removeClass("hover")
}).keyup(function(){d.text(c.find(":selected").text())
});
if($(c).attr("disabled")){f.addClass("disabled")
}})
}};$(function(){var a=(function(){var b=0;
return function(d,c){clearTimeout(b);
b=setTimeout(d,c)
}
})();
$("#txtSenha").after('<span id="forca-senha"/>').keyup(function(c){var b=$(this);
var e=$("#forca-senha");
var d=$("#forca-senha, #txtSenha");
d.attr("class","");
e.text("");
if(b.val().length===0){return
}if(b.val().length<8){d.addClass("invalido");
e.text("senha muito curta");
return
}a(function(){$.ajax({url:"/ajax/classificaSenha",dataType:"json",data:{senha:b.val()},type:"POST",success:function(h){var g=h.classificacao.forca;
if(g==4||g==3){d.addClass("forte");
e.html("n&iacute;vel da senha: forte")
}else{if(g==2||g==1){d.addClass("moderada");
e.html("n&iacute;vel da senha: moderada")
}else{if(g===0){d.addClass("invalido");
e.html("ops voc&ecirc; digitou caracteres n&atilde;o permitidos")
}}}}})
},1000)
});
$("#txtNovaSenha").on("change focusout",function(){var b='<span class="help-inline invalido">Voc&ecirc; n&atilde;o digitou a senha da mesma forma.<br />Por favor, tente novamente</span>';
$(this).next("span").remove();
if($(this).val()!=$("#txtSenha").val()){$(this).addClass("invalido").after(b)
}else{$(this).removeClass("invalido")
}});
$("#tip-holder").delegate(".link-seguranca-senha","click",function(){var b=$(window).width();
b=_isIPhoneOrIpodOrIpad()?window.innerWidth:b;
b=_isAndroid()?screen.width:b;
var d=100;
var c=(b-parseInt(540,10))/2;
$.blockUI({message:$("#seguranca-senha"),css:{top:d,left:c,cursor:"auto",width:"540px",height:"auto",padding:0,margin:0,border:"4px solid #e5e5e5","border-radius":"5px",textAlign:"left","z-index":"999999"},overlayCSS:{backgroundColor:"#fff",opacitykey:0.85}})
});
$(".box-seguranca-senha-fechar").click(function(){$.unblockUI();
return false
})
});$(function(){var a=(function(){var b=0;
return function(d,c){clearTimeout(b);
...
$("#login").change(function(){$("#login-status").remove();
$("#label-globomail").after('<span id="login-status" class="help-inline" />');
var b=$(this);
var c=$("#login-status");
b.parent("div").removeClass("error");
c.text("");
if(b.val().length===0){return
}if(b.val().length<4){b.parent("div").addClass("error");
c.text("login muito curto");
return
}a(function(){$.ajax({url:"/ajax/sugestaologin/"+escape(b.val()),dataType:"json",success:function(d){if(d.status=="invalido"){b.parent("div").addClass("error");
c.text("login inv&aacute;lido")
}else{if(d.sugestoes!==null){b.parent("div").addClass("error");
c.text("login indisponivel")
}}}})
},1000)
});
$("#tip-holder, span[for=email]").delegate("#link-alterna-login","click",ControleLogin.mostraGlobomailFree);
$("#tip-holder").delegate("#link-alterna-email","click",ControleLogin.mostraEmailLogin);
if($("#chaveLogin").val()=="LOGIN"){ControleLogin.mostraGlobomailFree()
}});
ControleLogin={mostraGlobomailFree:function(a){$("div.control-group.login, div.control-group.email_alternativo").removeClass("hidden");
$("div.control-group.email").addClass("hidden");
$("#tip-holder").hide();
$(".termo-servico-cadastro").addClass("hidden");
$(".termo-servico-globomailfree").removeClass("hidden");
$("#chaveLogin").val("LOGIN");
if(a){a.preventDefault()
}},mostraEmailLogin:function(a){$("div.control-group.login, div.control-group.email_alternativo").addClass("hidden");
$("div.control-group.email").removeClass("hidden");
$("#tip-holder").hide();
$(".termo-servico-cadastro").removeClass("hidden");
$(".termo-servico-globomailfree").addClass("hidden");
$("#chaveLogin").val("EMAIL");
if(a){a.preventDefault()
}}};Tooltip=function(a,c,b,d){this.activate=function(){var e=this;
$(a).bind("focus.tooltip",function(f){e.show($(this),f)
}).bind("blur.tooltip",function(f){e.hide($(this),f)
})
};
this.desactivate=function(){$(a).unbind(".tooltip")
};
this.enableUserInteraction=function(){var e=this;
$(c).bind("mouseover.tiphover",function(f){e.desactivate()
}).bind("mouseleave.tiphover",function(f){e.activate()
})
};
this.enableUserInteraction();
this.show=function(f,g){var e=this.tooltipPositionProperties(g,f);
this.updateContent(f);
e=this.makeItResponsive(f,e);
$(c).css("left",e.left()).css("top",e.top()).css("display","block")
};
this.makeItResponsive=function(f,e){f=this.findTargetElement(f);
if($(document).outerWidth()<=480){e.left=function(){return f.position().left
};
e.top=function(){return f.position().top+f.outerHeight()+13
};
$(c).addClass("tip_bottom").removeClass(b);
f.css("margin-bottom",($(c).outerHeight()+15)+"px")
}else{$(c).removeClass("tip_bottom").addClass(b)
}return e
};
this.updateContent=function(f){var e=this.generateContent(f);
$(c).find(".tiptip_content").html(e);
if(e.length<=20){$(c).css("width","130px")
}else{$(c).css("width","")
}};
this.generateContent=function(e){return $(e).parent().find(".restricoes").html()
};
this.hide=function(e){$(c).hide();
e.css("margin-bottom","")
};
this.findTargetElement=function(e){return $(e)
};
this.tooltipPositionProperties=function(k,e){var e=this.findTargetElement(e);
var g=e.position();
var j=function(){return -13
};
var f=function(){return 8
};
var i=function(){var l=e.outerWidth()+15;
return g.left+l
};
var h=function(){return g.top
};
if(b=="tip_top"){j=function(){return($(c).outerWidth()/2)-14
};
f=function(){return 43
}
}if(d=="mouse"){i=function(){var l=Math.round($(c).outerWidth());
return k.pageX-l/2
};
h=function(){return k.pageY-70
}
}return{arrow_left:j,arrow_top:f,left:i,top:h}
}
};